    ...
    <div class="container">
        ...
        <div class="row">
            @foreach ($clients as $client)
                {{-- @php
                $menuNamesString = App\Models\Menu::where('client_id', $client->id)->take(3)->pluck('name')->implode(', ');
            @endphp --}}
                <div class="col-md-3">
                    <div class="item pb-3">
                        <div class="list-card bg-white h-100 rounded overflow-hidden position-relative shadow-sm">
                            <div class="list-card-image">
                                ...
                                <a href="detail.html">
                                @endif
                                    <img src="{{ asset('uploads/client_profile/' . $client->image) }}"
                            ...
                            <div class="p-3 position-relative">
                                <div class="list-card-body">
                                    ...
                                    <p class="text-gray mb-3">
                                        @if ($client->menus->isNotEmpty())
                                            {{ $client->menus->implode(', ') }}
                                        @else
                                            No menus available
                                        @endif
                                    </p>
                                ...
                                <div class="list-card-badge">
                                    @if ($client->coupons)
                                    <span class="badge badge-success">OFFER</span> <small>{{$client->coupons->discount}}% off | Use Coupon
                                        {{ $client->coupons->name }}</small>
                                    @else
                                        <span class="badge badge-danger">No OFFER</span> <small></span>
                                    @endif
                                </div>
                            </div>
                        </div>
